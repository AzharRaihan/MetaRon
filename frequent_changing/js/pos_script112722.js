...
// purpose of this function is to delete data from sales table by using data from indexeddb
    ...
    function push_online_for_kitchen(order_object,is_self_order,sale_id,sale_no_new){
        $(".order_table_holder .order_holder").empty();
        let updated_sale_id = Number(sale_id);
        $("#update_sale_id").val('');
        $("#self_order_table_person").val('');
        let is_kiosk_order = $("#is_kiosk_order").val();
        if(updated_sale_id){
            if(s_width<=700){
                $.notifyBar({ cssClass: "success", html: "Order successfully updated!" });
            }
        }else if(is_kiosk_order=="Yes"){
            $.notifyBar({ cssClass: "success", html: "Order successfully added, your order number is "+sale_no_new });
...
// purpose of this function is to delete data from future sales table by using data from indexeddb
  ...
  function getCurrentDate() {
    ...
    $(document).keyup(function (e) {
        if ($("#search_barcode").is(":focus") && (e.keyCode == 13)) {
            e.preventDefault();
            let searched_string = $("#search_barcode").val().trim();
            if(searched_string){
                $("#search_barcode").val('');
                $(".prom_txt").html('');
                let modal_item_is_offer = '';
...
//get item/menu price from modal
                ...
                for (let i = 0; i < window.items.length; i++) {
                    // look for the entry with a matching `code` value
                    if (items[i].item_code == searched_string) {
                        is_exist = true;
                        tax_information = items[i].tax_information;
                        /*added_new_zakir*/
                        product_type = Number(items[i].product_type);
                        ...
                        get_qty = (items[i].get_qty);
                        modal_item_name_row = (items[i].modal_item_name_row);
                        price_delivery = (items[i].price_delivery);
                        price_sale = (items[i].price);
                        price_take = (items[i].price_take);
                        item_id = (items[i].item_id);
                        parent_id = (items[i].parent_id);
                        is_variation = (items[i].is_variation);
                    ...
                    if(is_exist==true){
                        if(status_continue==true){
                            if(is_variation=="Yes") {
                                ...
                                $("#modal_item_is_offer").html('Yes');
                                if (selected_order_type_object.attr("data-id") == "dine_in_button") {
                                    item_price = parseFloat(price_sale).toFixed(ir_precision);
                                }else if (selected_order_type_object.attr("data-id") == "take_away_button"){
                                    item_price = parseFloat(price_take).toFixed(ir_precision);
                                }else if (selected_order_type_object.attr("data-id") == "delivery_button"){
                                    let arr_item_details = search_by_menu_id(item_id, window.items);
                                    let check_dl_person = 1;
                        ...
                        toastr['error'](("No food menu found with your scanned barcode, you may need to add this food menu first."), '');
                    }
                }else{
                    $("#last_click_item_id").val(item_id);
                    $("#order_type_modal").addClass("active");
  ...
  $(document).on("click", ".edit_item", function () {
    ...
    $(document).on("click", "#place_order_operation", function (e) {
        ...
        if(is_login_checker==true){
            ...
            if(is_self_order!="Yes" || is_kiosk_order=="Yes"){
            ...
            if (selected_order_type_object.length > 0) {
                ...
                if (selected_order_type_object.attr("data-id") == "delivery_button") {
                    ...
                    order_type = 1;
                    if (waiter_id == "" && is_self_order!="Yes" && is_online_order!="Yes" && is_kiosk_order!="Yes") {
                        let op1 = $("#walk_in_customer").data("select2");
                        let op2 = $("#select_waiter").data("select2");
                        op1.close();
                        op2.open();
                ...
                let is_kiosk_order_1 = $("#is_kiosk_order").val();

                if(is_self_order=="Yes" || is_online_order=="Yes" || is_kiosk_order_1=="Yes"){
                    push_online_for_kitchen(order_info,'Yes',sale_id,sale_no_new);
                    clearFooterCartCalculation();
                }else{
                    ...
                    if(checkInternetConnection()){
                        if(pre_or_post_payment==1){
                            push_online_for_kitchen(order_info,'',sale_id,sale_no_new);
                        }
                    }
                }
        ...
        if (sale_id > 0) {
          ...
          if(is_split_bill!=1){
            ...
            let cart_modal_total_item_text = 0;
              ...
              if ($(".added_payment_cart").length > 0) {
                  let k = 1;
                  $(".added_payment_cart").each(function (i, obj) {
                      let payment_name = $(this).parent().find(".added_payment_cart_name").text();
                      let usage_point = 0;
          ...
          if (
              ...
              if (print_type_kot == "web_browser") {
                  let res_kot = get_all_information_from_indexeddb_kot_print(sale_id).then(function(data){
                      if(data.order !== null) {
                          print_kot_print(data.order,sale_id,data.kot_print);
                      } else {
                          console.log(sale_id + " => This id not found into the Database, Please reload your page");
                      }
                  });
      ...
      success: function (response) {
        ...
        if (notification_counter_update > notification_counter_previous) {
          ...
          setTimeout(function () {
            ...
            let is_kiosk_order = $("#is_kiosk_order").val();
            if(is_self_order!="Yes" && is_kiosk_order!="Yes"){
                if(is_online_order!="Yes" && is_kiosk_order!="Yes"){
                    bell_new_order.play();
                }
            }
        }
...
<div>
  ...
  $("body").on("click", ".removeCartItem", function () {
    ...
    if (current_status == "Started Cooking") {
        ...
        if(pos_7 || is_self_order=="Yes" || is_kiosk_order=="Yes"){
  ...
  $(document).on("click", "#last_ten_feature_button", function (e) {
    ...
    $.fn.lastTenFeatureButtonClick = function() {
    $("#is_self_order").val("No");
  ...
  win.on('click', function(e){
            ...
            if (
                ...
                let res = get_all_information_from_indexeddb_checker(sale_id).then(function(data){
                    ...
                    if(response !== null) {
                        ...
                        $(".dispaly-result").html(Number(response.total_payable).toFixed(ir_precision));
                        $(".finalize_total_payable").html(Number(response.total_payable).toFixed(ir_precision));
                        $(".finalize_total_payable").attr('data-original_payable',Number(response.total_payable).toFixed(ir_precision));
                        ...
                        $(".set_default_quick_cach").attr("data-amount",(Number(response.total_payable).toFixed(ir_precision)));
                        $(".set_default_quick_cach").html((Number(response.total_payable).toFixed(ir_precision)));
    ...
    $(document).on("click", "#order_payment_modal", function (e) {
        let this_value = $.trim($(this).val());
        if (isNaN(this_value)) {
            $(this).val("");
            $(".badge_custom").remove();
    ...
    function new_cart_cal(){
        let finalize_total_payable = Number($(".finalize_total_payable").text());
        let total_paid_amount = 0;
        $(".added_payment_cart").each(function (i, obj) {
            let this_amount = Number($(this).text());
            total_paid_amount+=this_amount;
        });
        let default_remaining_cash = finalize_total_payable-total_paid_amount;
        $(".finalize_total_due").text((default_remaining_cash).toFixed(ir_precision));
        ...
        $("#finalize_total_paid").html(total_paid_amount);
        $("#finalize_total_due").html(default_remaining_cash);
        $(".dispaly-result").html((default_remaining_cash).toFixed(ir_precision));

        if(!Number(default_remaining_cash)){
            generate_invoice();
    ...
    $(document).on("click", ".set_payment", function (e) {
        ...
        let status = true;
        if(status){
            let cal_amount = Number($(".dispaly-result").html());
            if(cal_amount){
                let payment_name = $(this).text();
                let html = '<tr>\n' +
                    '                                        <td class="added_payment_cart_name">'+payment_name+'</td>\n' +
                    '                                        <td style="text-align: right" data-id="'+id+'" class="added_payment_cart">'+cal_amount.toFixed(ir_precision)+'</td>\n' +
                    '                                    </tr>';
    ...
    function returnAmount(request_amount){
            let return_amount = 0;
            let finalize_total_payable = Number($(".finalize_total_payable").eq(0).text());
            let remaining_amount = Number($(".finalize_total_due").eq(0).text());
            return_amount = request_amount;
            if(request_amount>remaining_amount){
                let change_amount = request_amount - remaining_amount;
                return_amount = remaining_amount;
                $("#hidden_given_amount").val(finalize_total_payable+change_amount);
                $("#hidden_change_amount").val(change_amount);
            }
    ...
    $(document).on("click", ".get_quick_cash", function (e) {
        ...
        $(".added_payment_cart").each(function (i, obj) {
            let added_amount = Number($(this).text());
            let id = $(this).attr("data-id");
            if(id==1){
                let payment_name = $(this).parent().find(".added_payment_cart_name").text();
                 let cal_amount_tmp = returnAmount((amount));
                 cal_amount = ((cal_amount_tmp+added_amount));
                 $(this).text(cal_amount.toFixed(ir_precision));
                status = true;
            }

        });
        if(status==false){
            let payment_name = "Cash";
             cal_amount = returnAmount(amount,'');
            let html = '<tr>\n' +
                '                                        <td class="added_payment_cart_name">'+payment_name+'</td>\n' +
                '                                        <td style="text-align: right" data-id="1" class="added_payment_cart">'+cal_amount.toFixed(ir_precision)+'</td>\n' +
                '                                    </tr>';
            $("#payment_list").append(html);
        }
    ...
    $(document).on("click", ".reset_payment_details", function (e) {
        $("#payment_list").empty();
        new_cart_cal();
    });
    $(document).on("click", ".set_default_quick_cach", function (e) {
        let cal_amount = Number($(this).attr("data-amount"));
        let status = false;
        $(".added_payment_cart").each(function (i, obj) {
            let added_amount = Number($(this).text());
            let id = $(this).attr("data-id");
            ...
            if(id==1){
                let payment_name = $(this).parent().find(".added_payment_cart_name").text();
                let cal_amount_tmp = cal_amount+added_amount;

                $(this).text(cal_amount_tmp.toFixed(ir_precision));
                status = true;
            }
        ...
        let html = '<tr>\n' +
            '                                        <td class="added_payment_cart_name">Cash</td>\n' +
            '                                        <td style="text-align: right" data-id="1" class="added_payment_cart">'+cal_amount.toFixed(ir_precision)+'</td>\n' +
            '                                    </tr>';
        if(status==false){
