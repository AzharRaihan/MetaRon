...
$(document).ready(function () {
  ...
  function singleOrder(){
    ...
    $(document).on("click", "#add_spit_box", function (e) {
        ...
        if(spit_modal_input>maximum_spit){
            ...
            swal({
                title: warning + "!",
                text: (maximum_spit_is+" "+maximum_spit) + "!",
                confirmButtonText: ok,
                confirmButtonColor: "#b6d6f6",
            });
        }else{
    ...
    $(document).on("click", ".row_items_spit_plus", function (e) {
        ...
        if(qty>0){
            ...
            if(status==true){
                ...
                swal({
                    title: warning + "!",
                    text: please_select_a_box_on_right_side_for_assign_item + "!",
                    confirmButtonText: ok,
                    confirmButtonColor: "#b6d6f6",
                });
            }
    ...
    $(document).on("click", ".row_items_spit_minus", function (e) {
        ...
        if(status==true){
            ...
            if(status_box_item==true){
                ...
                swal({
                    title: warning + "!",
                    text: this_item_not_added_on_your_selected_customer + "!",
                    confirmButtonText: ok,
                    confirmButtonColor: "#b6d6f6",
                });
            }
        ...
        if(amount>0){
            if(status==true){
                if(Number(this_action.find('.row_box_item').length)){
                    ...
                    swal({
                        title: warning + "!",
                        text: you_need_to_add_at_least_1_item_on_right_selected_customer + "!",
                        confirmButtonText: ok,
                        confirmButtonColor: "#b6d6f6",
                    });
                }
    ...
    $(document).on("click", "#add_small_modal_amount", function (e) {
        ...
        if(spit_small_modal_input>tmp_total){
            ...
            swal({
                title: warning + "!",
                text: more_then_original_amount + "!",
                confirmButtonText: ok,
                confirmButtonColor: "#b6d6f6",
            });
        }else{
    ...
    $(document).on("click", ".goto_to_payment", function (e) {
        ...
        if($(this).parent().parent().find('.row_box_item').length) {
            ...
            if (status == true) {
                ...
                swal({
                    title: warning + "!",
                    text: some_of_your_cart_amounts_are_not_spitted_yet + "!",
                    confirmButtonText: ok,
                    confirmButtonColor: "#b6d6f6",
                });
            ...
            swal({
                title: warning + "!",
                text: please_add_at_least_1_item_before_checkout + "!",
                confirmButtonText: ok,
                confirmButtonColor: "#b6d6f6",
            });
        ...
        $.ajax({
            ...
            success: function (response) {
              if(response.status==true){
                  ...
                  $("#merge_bill_modal").addClass("active");
                  $(".pos__modal__overlay").fadeIn(200);
              }else{
                  swal({
                      title: warning + "!",
    ...
    $(document).on("click","#merge_now",function (e) {
      let this_action = $(this);
        if(Number($(".checkbox_user:checked").length)>=2){
            swal(
                {
                    title: warning + "!",
                ...
                function () {
                    let check_sale_ids = $(".checkbox_user:checked").map(function(){
                        return $(this).val();
                    }).get();
                    $.ajax({
                        ...
                        success: function (response) {
                          if(response.status==true){
                              $("#refresh_order").click();
                              $("#last_invoice_id").val(response.sale_id);
                              print_invoice(response.sale_id);
                              ...
                              swal({
                                  title: warning + "!",
                                  text: + "Something is wrong with merge, please try again!",
                                  confirmButtonText: ok,
                                  confirmButtonColor: "#b6d6f6",
                              });
                          }
            ...
            swal({
                title: warning + "!",
                text: "Please select more then two order for merge!",
                confirmButtonText: ok,
                confirmButtonColor: "#b6d6f6",
            });
        }
    ...
    $(document).on("click","#create_invoice_and_close",function (e) {
      ...
      if (
        ...
        let sale_id = $(
          ...
          .attr("id")
          .substr(6);
          let order_type = Number($(".holder .order_details .single_order[data-selected=selected]").attr("data-order_type"));
          $.ajax({
              url: base_url + "Sale/get_all_information_of_a_sale_ajax",
              method: "POST",
              data: {
  ...
  $(document).on("click", ".payment_method", function (e) {
    let this_value = $.trim($("#due_amount_invoice_input").val());
    if (isNaN(this_value)) {
      $(this).val("");
    }
    if (Number(this_value) > 0) {
      $(this).val(this_value);
    } else {
      $(this).val("");
    }
  ...
  function openProductEditModal(parent_id, item_name, id) {
    ...
    for (let key1 in foundItems_variations) {
      ...
      variations +=
        "<div class='vr01_modal_class' data-id='" +
      ...
      if (is_variation == "Yes") {
        ...
        if (!check_current_qty(item_id, qty_current) && has_kitchen == "No") {
          ...
          draw_table_for_order +=
            row_number > 0
              ? ""
              : '<div data-cp_type="1" class="single_order customer_panel fix" data-id="' +
                item_id +
                '" id="order_for_item_' +
                item_id +
                '">';
          ...
          draw_table_for_order +=
            '<div class="single_order_column first_column cart_item_counter  arabic_text_left fix" data-id="' +
            item_id +
            '"><i  data-parent_id="' +
            search_by_menu_id_getting_parent_id(item_id, window.items) +
            '"   class="fas fa-pencil-alt edit_item txt_5" id="edit_item_' +
            item_id +
          ...
          draw_table_for_order +=
            ...
            item_id +
            '"></i> <span class="qty_item_custom 1_cp_qty_' +
            item_id +
            ...
            '"></i></div>';
          draw_table_for_order +=
            '<div class="single_order_column forth_column fix"><input type="" name="" placeholder="Amt or %" class="1_cp_discount_' +
            item_id +
          ...
          if (note != "") {
            ...
            draw_table_for_order +=
              '<div class="single_order_column first_column cart_item_counter  arabic_text_left fix modifier_txt_custom"  data-id="' +
              item_id +
              '">' +
              note_txt +
  ...
  $(document).on("click", ".edit_item", function () {
    ...
    if (parent_id) {
      ...
      for (let key1 in foundItems_variations) {
        ...
        variations +=
          `<input ` +
          check_status +
          ` class="margin_for_vr01" name="vr01_name" type="radio"/>`;
        variations += "<p>" + foundItems_variations[key1].item_name + "</p>";
        variations +=
          '<span class="vr01_modal_price"> <span>' +
          price_txt +
    ...
    let cooking_status = single_order_element_object
      ...
      .html();
   /* if (cooking_status != "" && cooking_status !== undefined && cooking_status !== "New") {
        swal({
            title: warning + "!",
  ...
  $(document).on("click", "#cancel_order_button", function (e) {
    ...
    setTimeout(function () {
      $(".modal").removeClass("inActive");
      $(".cus_pos_modal_cancel").removeClass("inActive");
    }, 1000);
    $(".pos__modal__overlay").fadeOut(300);
  ...
  $(document).on("click", "#bill_order_action", function (e) {
    ...
    if (
      ...
      this_action
        ...
        .addClass("inActive");
      setTimeout(function () {
        this_action.parent().parent().parent().parent().removeClass("inActive");
      }, 1000);
      $(".pos__modal__overlay").fadeOut(300);

      let print_type_bill = $(".print_type_bill").val();
      let selected_language = $("#bill_language").val();
      $("#bill_language").val("english").change();
  ...
  $(document).on("click", "#cancel_order_action", function (e) {
    let cancel_reason_id = $(".cancel_reason_id:checked").val();
    let this_action = $(this);
    if (
      $(".holder .order_details > .single_order[data-selected=selected]")
        .length > 0
      ...
      swal(
        ...
        function () {
          ...
          $(".cus_pos_modal_cancel").removeClass("active").addClass("inActive");
          setTimeout(function () {
            $(".cus_pos_modal_cancel").removeClass("inActive");
          }, 1000);
          $(".pos__modal__overlay").fadeOut(300);
...
function update_all_total_price() {
  ...
  if (is_variation_product) {
    item_unit_price = parseFloat(
      $("#vr01_modal_price_variable:visible").html()
    ).toFixed(ir_precision);
  }
  //get item total price without discount
  let item_total_price_without_discount = (
...
function get_details_of_a_particular_hold(hold_id, this_action) {
  $.ajax({
    ...
    success: function (response) {
      ...
      if (Number(response.is_diplomat) == 2) {
        $("#is_diplomat").prop("checked", true);
      } else {
        $("#is_diplomat").prop("checked", false);
      }
        ...
        success: function (response) {
          ...
          for (let key in response) {
            ...
            order_list_left +=
              ...
              "</span></p>";
            order_list_left +=
              '<p>Order Type: <span class="running_order_table_name">' +
                order_type +
              "</span></p>";
            order_list_left +=
              '<p>Table: <span class="running_order_table_name">' +
...
function reset_finalize_modal() {
  ...
  $("#finalie_order_invoice_language").val("english").change();
...
function arrange_info_on_the_cart_to_modify(response) {
  ...
  for (let key in response.items) {
    ...
    draw_table_for_order +=
      ...
      '">' +
      this_item.menu_price_with_discount +
      "</span><i data-row_id='"+this_item.id+"' data-id='" +
      this_item.food_menu_id +
      "' class='fal fa-times removeCartItem'></i></div>";
    draw_table_for_order += "</div>";
    if (selected_modifiers != "") {
        ...
        $.ajax({
            ...
            success: function (response) {
                  if(response.status==false){
                      ...
                      this_action.parent().parent().parent().parent().removeClass("active").addClass("inActive");
                      setTimeout(function () {
                          $(".modal").removeClass("inActive");
                      }, 1000);
                      $("#cancel_modal_void_comps").addClass("active");
                      $(".pos__modal__overlay").fadeIn(200);
                  }
            },
            error: function () {
...
$(document).on("click", "#cancel_void_comps_action", function (e) {
    let cancel_reason_void_comps = $(".cancel_reason_void_comps:checked").val();
    ...
    swal(
        ...
        function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                  $(".order_for_item_"+inline_row_remove_id).remove();
                    setNumberRemove1();
                    swal({
                        title: warning,
                        text: response.msg,
...
$(document).on("click", ".is_diplomat", function (e) {
    ...
    if ($("#is_diplomat").prop("checked") == false) {
