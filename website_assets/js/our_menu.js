$(function() {
	...
	$(document).on('submit', '#form_update', function (e) {
		...
		let menu_img_2 = $("#menu_img_2").val();
		let put_old_value_2 = $("#put_old_value_2").val();
		let menu_img_3 = $("#menu_img_3").val();
		...
		if (menu_img_1 == "" && put_old_value_1 == "") {
			$("#menu_img_1_err_msg").text(image_field_required);
			$(".menu_img_1_err_msg_contnr").show(200);
			error = true;
		}
		if (menu_img_2 == "" && put_old_value_2 == "") {
			$("#menu_img_2_err_msg").text(image_field_required);
			$(".menu_img_2_err_msg_contnr").show(200);
			error = true;
		}
		if (menu_img_3 == "" && put_old_value_3 == "") {
			$("#menu_img_3_err_msg").text(image_field_required);
			$(".menu_img_3_err_msg_contnr").show(200);
			error = true;
		}
		if (status == "") {
		...
		if (error == false){
			$.ajax({
				type: "POST",
				url: base_url+'WebsiteAdminOurMenu/updateOurMenu/1',
				dataType: "JSON",
				processData: false,
				contentType: false,
				cache: false,
