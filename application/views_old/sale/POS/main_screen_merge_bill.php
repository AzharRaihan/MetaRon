...
foreach($food_menus as $single_menus){
    ...
    $menu_to_show .= '<div class="single_item animate__animated animate__flipInX" data-price="'.$sale_price.'"  data-is_variation="'.$is_variation.'"  id="item_'.$single_menus->id.'">';
    ...
    if($single_menus->parent_id!='0'){
        $item_name_tmp = getParentNameTemp($single_menus->parent_id).(isset($single_menus->name) && $single_menus->name?''.$single_menus->name:'');
    }else{
        $item_name_tmp = $single_menus->name;
    }


    if($total_menus==$i){
        $javascript_obects .= "{item_id:'".$single_menus->id."',item_code:'".(str_replace("'", ' ', str_replace('"', ' ', $single_menus->code)))."',category_name:'".(str_replace("'", ' ', str_replace('"', ' ', $single_menus->category_name)))."',is_variation:'".$is_variation."',item_name:'".(str_replace("'", ' ', str_replace('"', ' ', $single_menus->name)))."',item_name_tmp:'".(str_replace("'", ' ', str_replace('"', ' ', $item_name_tmp)))."',parent_id:'".$single_menus->parent_id."',price:'".getAmtP($sale_price)."',sale_price_tmp:'".getAmtP($single_menus->sale_price_tmp)."',image:'".$image_path."',tax_information:'".$single_menus->tax_information."',vat_percentage:'0',veg_item:'".$veg_status."',beverage_item:'".$soft_status."',bar_item:'".$bar_status_pos."',sold_for:'".$single_menus->item_sold."',veg_item_status:'".$veg_status1."',beverage_item_status:'".$beverage_status."',bar_item_status:'".$bar_status_pos."',modifiers:[".$modifiers."]}";
    }else{
        $javascript_obects .= "{item_id:'".$single_menus->id."',item_code:'".(str_replace("'", ' ', str_replace('"', ' ', $single_menus->code)))."',category_name:'".(str_replace("'", ' ', str_replace('"', ' ', $single_menus->category_name)))."',is_variation:'".$is_variation."',item_name:'".(str_replace("'", ' ', str_replace('"', ' ', $single_menus->name)))."',item_name_tmp:'".(str_replace("'", ' ', str_replace('"', ' ', $item_name_tmp)))."',parent_id:'".$single_menus->parent_id."',price:'".getAmtP($sale_price)."',sale_price_tmp:'".getAmtP($single_menus->sale_price_tmp)."',image:'".$image_path."',tax_information:'".$single_menus->tax_information."',vat_percentage:'0',veg_item:'".$veg_status."',beverage_item:'".$soft_status."',bar_item:'".$bar_status_pos."',sold_for:'".$single_menus->item_sold."',veg_item_status:'".$veg_status1."',beverage_item_status:'".$beverage_status."',bar_item_status:'".$bar_status_pos."',modifiers:[".$modifiers."]},";
    }
...
foreach($tables as $table){
    $tables_modal .= '<div class="floatleft fix single_order_table area_all_table area_table_'.$table->area.'" id="single_table_info_holder_'.$table->id.'">';
    ...
    $tables_modal .= '<div class="header">';
    $tables_modal .= '<div class="floatleft fix column first_column" style="width: 100%">';
        $select_option_for_table = '<select class="order_table_select form-control" id="order_table_select'.$table->id.'" data-table_name="'.$table->name.'"  data-table_id="'.$table->id.'" style="border: 0px;height: 40px;width: 100%;">';
        $select_option_for_table .= '<option value="">Select Guest</option>';
        for ($m=1;$m<=$table->sit_capacity;$m++):
            $select_option_for_table .= '<option value="'.$m.'">'.$m.'</option>';
        endfor;;
        $select_option_for_table .= '</select>';

    $tables_modal .=$select_option_for_table;
    $tables_modal .= '</div>';
    $tables_modal .= '</div>';
    $tables_modal .= '</div>';
    ...
    $tables_modal .= '</div>';
    $tables_modal .= '<div><button style="background-color: #00a2e8;color:white" class="proceed_to_payment">'.lang("proceed_to_payment").'</button></div></div>';
    $tables_modal .= '</div>';
    $i++;
}
/********************************************************************************************************************
...
foreach($new_orders as $single_new_order){
    ...
    if($i==1){
        $order_list_left .= '<div data-order_type="'.$single_new_order->order_type.'" data-started-cooking="'.$total_kitchen_type_started_cooking_items.'" data-done-cooking="'.$total_kitchen_type_done_items.'" class="txt_14 single_order new_order_'.$single_new_order->sales_id.'" data-selected="unselected" id="order_'.$single_new_order->sales_id.'">';
    }else{
        $order_list_left .= '<div data-order_type="'.$single_new_order->order_type.'" data-started-cooking="'.$total_kitchen_type_started_cooking_items.'" data-done-cooking="'.$total_kitchen_type_done_items.'" class="single_order new_order_'.$single_new_order->sales_id.'" data-selected="unselected" id="order_'.$single_new_order->sales_id.'">';
    }
    ...
    $tables_booked = $single_new_order->orders_table_text;
    ...
    $order_list_left .= '<p>'.lang('order_type').': <span class="running_order_type">'.$order_type.'</span></p>';
...
<body>
    ...
    <input type="hidden" id="auto_print_kot" value="<?php echo escape_output($this->session->userdata('auto_print_kot'))?>">
    <input type="hidden" id="auto_print_bot" value="<?php echo escape_output($this->session->userdata('auto_print_bot'))?>">
    <input type="hidden" id="outlet_id_pos" value="<?php echo escape_output($this->session->userdata('outlet_id'))?>">
    <input type="hidden" id="company_id_pos" value="<?php echo escape_output($this->session->userdata('company_id'))?>">
    ...
    <input type="hidden" id="purchase_txt" value="<?php echo lang('purchase'); ?>">
    <input type="hidden" id="expanse_txt" value="<?php echo lang('expense'); ?>">
    <input type="hidden" id="supplier_payment_txt" value="<?php echo lang('supplier_payment'); ?>">
    ...
    <input type="hidden" id="proceed_to_payment" value="<?php echo lang('proceed_to_payment'); ?>">
    ...
    <input type="hidden" id="inv_delivery_charge" value="<?php echo lang('delivery_charge') ?>">
    ...
    <input type="hidden" name="print_receipt_on_close_order" class="print_receipt_on_close_order" id="print_receipt_on_close_order" value="<?php echo isset($getCompanyInfo->print_receipt_on_close_order) && $getCompanyInfo->print_receipt_on_close_order?$getCompanyInfo->print_receipt_on_close_order:'No'; ?>">
    <input type="hidden" name="is_ignore_change" class="is_ignore_change" id="is_ignore_change" value="">
    <div class="preloader">
            ...
            <div class="left_item">
                <div class="header_part_middle">
                    <ul class="icon__menu">
                        ...
                        <li>
                            <a href="#" class="split_bill">
                                <?php  echo lang('split_bill');?>
                            </a>
                        </li>
                        <li>
                            <a href="#" class="merge_bill">
                                <?php  echo lang('merge_bill');?>
                            </a>
                        </li>
                    </ul>
            ...
            <div class="header_part_right">
                <ul class="btn__menu">
                    ...
                    <li><a href="#" data-status="bar"
                           class="veg_bev_item bg__khoyre"><?php echo lang('bar'); ?></a></li>
                </ul>
            </div>
        </div>
                ...
                <div class="main_left">
                    <div class="holder">
                        ...
                        <div class="ir_pa_b_w100" id="left_side_button_holder_absolute">
                            ...
                            <button class="no-need-for-waiter operation_button fix btn_tip"
                                    id="print_kot"
                                    data-tippy-content="<?php echo lang('print_kot'); ?>">
                                <i class="fas fa-print"></i> <?php echo lang('print_kot'); ?>
                            </button>

                            <?php if($getCompanyInfo->pre_or_post_payment == "Post Payment"){?>
                            <div class="ir_flex_jc_w94_pr">
                                <div class="invoice_box">
                                    <button class="invoice_btn_class" data-type="3"><?php echo lang('merge_bills'); ?></button>
                            ...
                            if(isset($waiter_app_status) && $waiter_app_status=="Yes" && $getCompanyInfo->printing_kot!="web_browser"):
                                ?>
                                <button class="operation_button fix  full-width-for-waiter"
                                        id="print_kot"
                                        data-tippy-content="<?php echo lang('print_kot'); ?>">
                                    <i class="fas fa-print"></i> <?php echo lang('print_kot'); ?>
                                </button>
                            ...
                            ?>
                            <?php if($getCompanyInfo->pre_or_post_payment == "Post Payment"){?>
                            <button class="operation_button no-need-for-waiter fix" style="display: none"><i
                                    class="fas fa-ban"></i>
                ...
                <div class="main_middle">
                    <div class="main_top fix">
                        ...
                        <div class="button_holder no-need-for-waiter">
                               <?php
                                   $order_type_din = $this->session->userdata('order_type_din');
                                   $order_type_take = $this->session->userdata('order_type_take');
                                   $order_type_delivery = $this->session->userdata('order_type_delivery');
                               ?>
                            <button  style="display: <?php echo isset($order_type_din) && $order_type_din=="No"?'none':'' ?>"  class="selected__btn_c"
                                data-selected="<?php echo isset($waiter_app_status) && $waiter_app_status=="Yes"?'selected':''?>"
                                id="dine_in_button">
                                <i class="fal fa-table"></i> <?php echo lang('dine'); ?>
                            </button>

                            <button style="display: <?php echo isset($order_type_take) && $order_type_take=="No"?'none':'' ?>" class="selected__btn_c" id="take_away_button"><i class="fal fa-shopping-bag"></i>
                                <?php echo lang('take_away'); ?></button>

                            <button  style="display: <?php echo isset($order_type_delivery) && $order_type_delivery=="No"?'none':'' ?>"  class="selected__btn_c" id="delivery_button"><i class="fal fa-truck"></i>
    ...
    <div class="all__menus">
        <ul class="menu__list">
            ...
            <div>
                ...
                <li class="no-need-for-waiter">
                    <a href="#" class="register_close_send">
                        <i class="fal fa-envelope"></i> Send <?php echo lang('register'); ?>
                    </a>
                </li>
                <li class="no-need-for-waiter">
        ...
        <div class="modal-content">
            ...
            <div class="main-content-wrapper">
                <div class="content">
                    ...
                    <span><label style="margin-left: -4px;" class="is_diplomat"><input type="checkbox" name="is_diplomat" value="1" id="is_diplomat"> <?php echo lang('is_diplomat'); ?></label></span>
                </div>
    ...
    <div id="item_modal" class="modal">
        ...
        <div class="modal-content">
            ...
            <div class="ir_mrx5">
                ...
                <div class="section3 section3_new fix">
            ...
            <div class="customer_add_modal_info_holder">
                <div class="content">
                    ...
                    <div class="left-item b">
                        ...
                        <input type="hidden" id="remove_vat" value="">
            ...
            <div class="select_table_modal_info_holder2">
                ...
                <div class="table-category-list">
                    <ul class="dineIn-table-list-of-item">
                        <li>
                            <a href="javascript:void(0)" class="get_area_table" data-id="all"><b><?php echo lang('Floors'); ?></b></a>
                        </li>
                        <?php
                        foreach ($areas as $value) {
                            echo '<li><a class="get_area_table" data-id="'.$value->id.'" href="javascript:void(0)">'.$value->floor_name.'</a></li>';
                        }
                        ?>
                    </ul>
                </div>
                <div class="all-dineIn-table scrollbar-macosx">
    ...
    <div id="modal_void" class="modal">
        ...
        <!-- Modal content -->
        <div class="modal-content">
            <h1 class="main_header fix"> Select Your Option
                <a href="javascript:void(0)" class="alertCloseIcon">
                    <i class="fal fa-times"></i>
                </a>
            </h1>
            <div class="modal_void fix">
                <div class="btn_box" style="text-align: center;margin-top: 6px;">
                    <button type="button" style="padding: 0px 40px;" data-id="1" class="bg__green action_void_comps">Void</button>
                    <button type="button" style="padding: 0px 40px;" data-id="2" class="bg__green action_void_comps">Comps</button>
                    <br>
                    <button type="button" style="padding: 0px 40px;margin-top: 9px;" class="bg__red cancel_modal">Cancel</button>
                </div>
            </div>
        </div>

    </div>
    <div id="modal_void_enter" class="modal">

        <!-- Modal content -->
        <div class="modal-content">
            <h1 class="main_header fix"> Enter Your Code
                <a href="javascript:void(0)" class="alertCloseIcon">
                    <i class="fal fa-times"></i>
                </a>
            </h1>
            <div class="modal_void_enter fix">
                <div class="btn_box" style="text-align: center;margin-top: 6px;">
                    <input type="hidden" id="comp_and_void_code_type" value="">
                    <input type="hidden" id="remove_row_id" value="">
                    <input type="hidden" id="inline_row_remove_id" value="">
                    <input type="text" value="" style="height: 34px;width: 80%;margin-bottom: 9px;" class="form-control" name="comp_and_void_code" id="comp_and_void_code">
                    <br>
                    <button type="button" style="padding: 0px 30px;" id="check_void_comps_action" class="bg__green">Yes</button>
                    <button type="button" style="padding: 0px 30px;" class="bg__red cancel_modal">No</button>
                </div>
            </div>
        </div>
    ...
    <div class="cus_pos_modal cus_pos_modal_cancel" id="cancel_modal_void_comps">
        <header class="pos__modal__header">
            <h3 class="pos__modal__title">Cancel Reasons</h3>
            <a href="javascript:void(0)" class="pos__modal__close"><i class="fal fa-times"></i></a>
        </header>
        <div class="pos__modal__body">
            <div class="default_inner_body default_inner_body_cancel">
                <table style="border-collapse: collapse;border: 1px solid #ababab;width: 100%">
                    <?php foreach ($cancellation_reasons as $key=>$value):
                        ...
                        ?>
                        <tr style="border: 1px solid #ababab;cursor: pointer;">
                            <td style="padding: 8px 0px 8px 0px;cursor: pointer;"><label style="cursor: pointer"><input <?php echo $checked?> type="radio" name="cancel_reason_void_comps" class="cancel_reason_void_comps" value="<?php echo $value->name; ?>"><b><?php echo $value->name; ?></b></label></td>
                        </tr>
                    <?php endforeach;?>
                </table>
                <div style="text-align: center;"><button id="cancel_void_comps_action" class="custom_btn"> <?php echo lang('submit'); ?> </button></div>
            </div>
    ...
    <div class="cus_pos_modal" id="register_modal">
        ...
        <footer class="pos__modal__footer">
            <div class="right_box">
                ...
                <button type="button" class="register_close_send"> Send <?php echo lang('register'); ?></button>
    ...
    <div class="cus_pos_modal cus_pos_modal_cancel" id="cancel_modal">
        <header class="pos__modal__header">
            <h3 class="pos__modal__title"><?php echo lang('close_without_payment'); ?></h3>
            <a href="javascript:void(0)" class="pos__modal__close"><i class="fal fa-times"></i></a>
        </header>
        <div class="pos__modal__body">
            <div class="default_inner_body default_inner_body_cancel">
                <table style="border-collapse: collapse;border: 1px solid #ababab;width: 100%">
                    <?php foreach ($cancellation_reasons as $key=>$value):
                        ...
                        ?>
                        <tr style="border: 1px solid #ababab;cursor: pointer;">
                            <td style="padding: 8px 0px 8px 0px;cursor: pointer;"><label style="cursor: pointer"><input <?php echo $checked?> type="radio" name="cancel_reason_id" class="cancel_reason_id" value="<?php echo $value->name; ?>"><b><?php echo $value->name; ?></b></label></td>
                        </tr>
                    <?php endforeach;?>
                </table>
                <div style="text-align: center;"><button id="cancel_order_action" class="custom_btn"> <?php echo lang('close_without_payment'); ?> </button></div>
            </div>

        </div>
    </div>

    <div class="cus_pos_modal cus_pos_modal_bill" id="bill_modal_print">
        <header class="pos__modal__header">
            <h3 class="pos__modal__title"><?php echo lang('select_language'); ?></h3>
            <a href="javascript:void(0)" class="pos__modal__close"><i class="fal fa-times"></i></a>
        </header>
        <div class="pos__modal__body">
            <div class="default_inner_body default_inner_body_cancel">
                <select name="bill_language"  style="width: 100%"  class="select2" id="bill_language">
                    <!--This variable could not be escaped because this is html content-->
                    <?php
                    $dir = glob("application/language/*",GLOB_ONLYDIR);
                    foreach ($dir as $value):
                        ...
                        ?>
                        <option <?php echo isset($language) && $language==$separete[1]?'selected':''?> value="<?php echo ($separete[1])?>"><?php echo ucfirst($separete[1])?></option>
                        <?php
                    endforeach;
                    ?>

                </select>
                <div style="text-align: center;"><button id="bill_order_action" class="custom_btn_green"> <?php echo lang('print'); ?> </button></div>
            </div>
    ...
    <div id="finalize_order_modal" class="modal">
        ...
        <div class="modal-content" id="modal_finalize_order_details">
            ...
            <div class="fo_3 fix">
            <?php
            foreach ($payment_methods as $ky=>$payment_method):
                    $style = '';
                    if($ky!=0){
                     $style = "margin-top: 13px;";
                    }
                ?>
                <div class="half fix floatleft textleft" style="<?php echo $style?>" ><b><?php echo $payment_method->name; ?></b></div>
                <div class="half fix floatleft textleft" style="<?php echo $style?>" ><input data-payment_id="<?php echo $payment_method->id; ?>" onfocus="select();" type="text" name="payment_method[]" class="payment_method"></div>
            <?php endforeach;?>
            </div>
            ...
            <div class="fo_3 fix">
                ...
                <div class="half fix floatleft textleft"><input type="text" readonly name="pay_amount_invoice_modal_input"
            ...
            <div class="fo_2 fix">
                <div class="half fix floatleft" style="    padding-top: 8px;"><?php echo lang('invoice_language'); ?></div>
                <div class="half fix floatleft">
                    <select name="finalie_order_invoice_language" class="select2" id="finalie_order_invoice_language">
                        <!--This variable could not be escaped because this is html content-->
                        <?php
                        $dir = glob("application/language/*",GLOB_ONLYDIR);
                        foreach ($dir as $value):
            ...
            <div class="bottom_buttons fix">
                <div class="bottom_single_button fix">
                    <button id="finalize_order_button"><?php echo lang('pay'); ?></button>
                </div>
                <div class="bottom_single_button fix">
                    <button id="finalize_order_cancel_button"><?php echo lang('cancel'); ?></button>

                    <b><a href="#" id="cancel_order_button" class="close_without_payment" style="color:#e32b2b;text-decoration: none;"><?php echo lang('close_without_payment'); ?></a> </b>
                </div>
            </div>
            <!-- <span class="btn-close">&times;</span> -->
            <!-- <p>Some text in the Modal..</p> -->
    ...
    <div id="merge_bill_modal" class="modal">
        ...
        <div class="modal-content" id="modal_notification_list_details">
            ...
            <div id="notification_list_header_holder">
                <div class="single_row_notification_header fix ir_h25_bb1">
                    <label class="fix single_notification pointer_merge"><strong><input type="checkbox" class="checkbox_userAll" id="checkbox_userAll"><?php echo lang('select_all'); ?></strong></label>
                    <div class="fix single_serve_button">
                    </div>
                </div>
            </div>
            ...
            <!-- <p>Some text in the Modal..</p> -->
            <div id="notification_close_delete_button_holder">
                <button id="merge_now"><?php echo lang('merge'); ?></button>
                <button id="notification_close"><?php echo 'Cancel';//lang('close'); ?></button>
            </div>
        </div>
    ...
    <div id="kot_list_modal" class="modal">
        ...
        <div class="modal-content" id="modal_kot_list_details">
            ...
            <div id="kot_bottom_buttons" class="fix">
                ...
                <button id="print_kot_modal"><?php echo lang('print_kot'); ?></button>
            </div>
    ...
    <aside id="pos__sidebar">
        ...
        <ul class="pos__menu__list scrollbar-macosx">
            ...
            <li class="have_sub_menu">
                <a class="open-trigger" href="javascript:void(0)" href="#">
            ...
            <li class="have_sub_menu">
                ...
                <ul class="sub__menu__list">
                    ...
                    if(isset($value_menu1->sub_sub_menus) && $value_menu1->sub_sub_menus):
                        ?>
                        <li class="have_sub_sub_menu">
                            <a class="open-trigger" href="#">
                                <div>
                                    <i class="fa fa-angle-double-right"></i>
                                    <?php echo lang($value_menu1->label); ?>
                                </div>
                                <span class="pull-right-container">
                                <i class="far fa-chevron-left"></i>
                            </span>
                            ...
                            <ul class="sub__sub__menu__list">
                                <?php  foreach ($value_menu1->sub_sub_menus as $ky11=>$value_menu11):
                                    $base_url_g1_ = base_url().$value_menu11->controller_name."/".$value_menu11->function_name;
                                    ?>
                                    <li class="menu_assign_class">
                                        <a href="<?php echo $base_url_g1_?>"><i class="fa fa-angle-double-right"></i> <?php echo lang($value_menu11->label); ?></a>
                                    </li>
                                <?php endforeach;?>
                            </ul>
                        </li>
    ...
    <?php
    $tax_info = (Object)json_decode($getCompanyInfo->tax_setting);
    $tax_information_charge = array();
    foreach ($tax_info as $tax){
        $single_info = array(
            'tax_field_id' => $tax->id,
            'tax_field_company_id' => $this->session->userdata('company_id'),
            'tax_field_name' => $tax->tax,
            'tax_field_name_arabic' => $tax->tax,
            'tax_field_percentage' => $tax->tax_rate,
        );
        array_push($tax_information_charge,$single_info);
    }
    ?>
    ...
    <div class="ir_display_none total_split_sale"></div>
    ...
    <div style="display: none" class="charge_tax_html" id="charge_tax_html"><?php echo json_encode($tax_information_charge)?></div>
    ...
    $(document).on('click', '.register_close_send', function(e) {
        let base_url_s = $("base").attr("data-base");
        let html_header = $("#opening_closing_register_time").text();
        let html_content = $("#register_details_content").html();
        $.ajax({
            url: base_url_s + "Sale/sendRegisterEmail",
            method: "POST",
            dataType:'json',
            data: {
