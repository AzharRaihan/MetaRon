...
class Waste extends Cl_Controller {
    ...
    public function __construct() {
        ...
        if($segment_2=="wastes"){
            ...
            $function = "add";
        }elseif($segment_2=="deleteWaste" || $segment_2=="deleteWasteNew"){
            $function = "delete";
        }else{
    ...
    public function deleteWasteNew() {

        $id = $this->input->post('dataid');

        $this->db->set('del_status', "Deleted");
        $this->db->where('id', $id);
        $this->db->update('tbl_wastes');

        $this->db->set('del_status', "Deleted");
        $this->db->where('waste_id', $id);
        $result = $this->db->update('tbl_waste_ingredients');
