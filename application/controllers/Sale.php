...
class Sale extends Cl_Controller {
    ...
    public function resetDailySales() {
        ...
        $this->db->query("TRUNCATE tbl_orders_table");
    ...
    public function POS($user_id='', $outlet_id=''){
        ...
        if($is_kiosk_order=="Yes"){
            $this->load->view('sale/POS/kiosk_screen', $data);
        }else{
    ...
    public function add_kitchen_sale_by_ajax(){
        ...
        $data['order_receiving_id'] = getOrderReceivingId($this->session->userdata('outlet_id'));
        ...
        if($sale_id>0){
            ...
            if($is_self_order=="Yes" && $is_online_order!="Yes" && $is_kiosk_order!="Yes"){
            ...
            if($is_kiosk_order=="Yes"){
                $notification = "a new kiosk order has been placed, Order Number is: ".$sale_no;
                $notification_data = array();
                $notification_data['notification'] = $notification;
                $notification_data['sale_id'] = $sale_id;
                $notification_data['waiter_id'] = trim($order_details->waiter_id);
    ...
    public function push_online(){
        ...
        if($sale_id>0){
            ...
            $order_number = '';
            if($check_existing->order_type==1){
                $order_number = $check_existing->sale_no;
            }else if($check_existing->order_type==2){
                $order_number = $check_existing->sale_no;
            }else if($check_existing->order_type==3){
                $order_number = $check_existing->sale_no;
            }
