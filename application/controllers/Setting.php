...
class Setting extends Cl_Controller {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if($outlet_id){
                    ...
                    foreach ($tables as $value){
                        ...
                        $save_path = "qr_code/";
                        $file_name1 = $save_path."table-qrcode-".$value->id.".png";

                        $file_name = $file_name1;
                        QRcode::png($url_qrcode,$file_name,'',4,1);
    ...
    public function kioskSetting($id = '') {
        $outlet_id_post = htmlspecialchars($this->input->post($this->security->xss_clean('outlet_id')));
        $outlet_id = isset($outlet_id_post) && $outlet_id_post?$outlet_id_post:$this->session->userdata('outlet_id');
        if ($this->input->post('submit')) {
            if(isLMni()):
            ...
            if ($this->form_validation->run() == TRUE) {
                $data = array();
                $data['main_content'] = $this->load->view('authentication/kioskSetting', $data, TRUE);
                $this->load->view('userHome', $data);
